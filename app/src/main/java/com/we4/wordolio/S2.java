...
public class S2 extends AppCompatActivity {
    ...
    public void ok(View view)
    {
        try
        {
            ...
            {
                for(i=0;i<n;i++)
                    if(w.charAt(i)=='a' || w.charAt(i)=='x' || w.charAt(i)=='e' || w.charAt(i)=='s')
                        count=count+1;
                if(w.equals("axes") && count==4)
                {
                    score=score+100;
                    LocalDB.getInstance().insertToSimpleScore(2,score);
                    LocalDB.getInstance().updateSimpleScoreStatus(2,score);
                    LocalDB.getInstance().updateSimpleStatus(3,"true");
                    AlertDialog.Builder ad = new AlertDialog.Builder(S2.this);
                    ...
                    ad
                            ...
                            .setPositiveButton("Continue",new DialogInterface.OnClickListener(){
                                public void onClick(DialogInterface dialog,int id){
                                    Intent pos=new Intent(S2.this,Simple.class);
                                    startActivity(pos);
                                    finish();
