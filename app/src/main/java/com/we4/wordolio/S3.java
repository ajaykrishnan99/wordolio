...
public class S3 extends AppCompatActivity {
    ...
    public void ok(View view)
    {
        try
        {
            ...
            {
                for(i=0;i<n;i++)
                    if(w.charAt(i)=='b' || w.charAt(i)=='l' || w.charAt(i)=='u' || w.charAt(i)=='n' || w.charAt(i)=='t')
                        count=count+1;
                if(w.equals("blunt") && count==5)
                {
                    score=score+100;
                    LocalDB.getInstance().insertToSimpleScore(3,score);
                    LocalDB.getInstance().updateSimpleScoreStatus(3,score);
                    LocalDB.getInstance().updateSimpleStatus(4,"true");
                    AlertDialog.Builder ad = new AlertDialog.Builder(S3.this);
                    ...
                    ad
                            ...
                            .setPositiveButton("Continue",new DialogInterface.OnClickListener(){
                                public void onClick(DialogInterface dialog,int id){
                                    Intent pos=new Intent(S3.this,Simple.class);
                                    startActivity(pos);
                                    finish();
