...
public class S4 extends AppCompatActivity {
    ...
    public void ok(View view)
    {
        try
        {
            ...
            {
                for(i=0;i<n;i++)
                    if(w.charAt(i)=='m' || w.charAt(i)=='a' || w.charAt(i)=='g' || w.charAt(i)=='i' || w.charAt(i)=='c')
                        count=count+1;
                if(w.equals("magic") && count==5)
                {
                    score=score+100;
                    LocalDB.getInstance().insertToSimpleScore(4,score);
                    LocalDB.getInstance().updateSimpleScoreStatus(4,score);
                    LocalDB.getInstance().updateSimpleStatus(5,"true");
                    AlertDialog.Builder ad = new AlertDialog.Builder(S4.this);
                    ...
                    ad
                            ...
                            .setPositiveButton("Continue",new DialogInterface.OnClickListener(){
                                public void onClick(DialogInterface dialog,int id){
                                    Intent pos=new Intent(S4.this,Simple.class);
                                    startActivity(pos);
                                    finish();
